...
def test_substring_pattern():
    ...
    assert pattern.matches("adder.v") == False, "Substring 'alu' should not match 'adder.v'"
    assert pattern.exact_match("alu.v") == True, "Exact match for 'alu.v' should be True"
    assert pattern.exact_match("alu_adder.v") == False, "Exact match for 'alu_adder.v' should be False"
...
def test_substring_count_pattern():
    ...
    assert pattern.matches("alu_alu_alu.v") == False, "3 occurrences of 'alu' should not match"
    assert pattern.matches("adder.v") == False, "0 occurrences of 'alu' should not match"
...
def test_string_match_pattern():
    ...
    assert pattern.matches("alu_adder.v") == False, "String 'alu_adder.v' should not match 'alu.v'"
    assert pattern.matches("alu.v.bak") == False, "String 'alu.v.bak' should not match 'alu.v'"
