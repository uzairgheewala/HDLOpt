...
class RuleTestFactory:
    ...
    def _discover_rules(self) -> None:
        ...
        rule_files = [f for f in rules_path.glob("*.py") 
    ...
    def run_tests(self) -> Dict[str, List[TestResult]]:
        ...
        for rule_name, rule_class in self.rules.items():
            ...
            for test_case in test_cases:
                ...
                error_msg = None
                if not passed:
                    error_msg = (f"Test failed: inputs={test_case.inputs}, "
                               f"expected={test_case.expected_outputs}, "
                               f"actual={actual_outputs}")

                rule_results.append(TestResult(
                    test_case=test_case,
                    passed=passed,
        ...
        for rule_name, rule_class in self.rules.items():
            ...
            if failed_tests:
                ...
                for i, result in enumerate(failed_tests[:5], 1):
                    ...
                    pdf_gen.add_paragraph(f"Expected: {result.test_case.expected_outputs}")
                    pdf_gen.add_paragraph(f"Actual: {result.test_case.actual_outputs}")
                    pdf_gen.add_paragraph("---")
                
                if len(failed_tests) > 5:
                    pdf_gen.add_paragraph(f"(Showing 5 of {len(failed_tests)} failed tests)")
...
def test_rule_discovery(rule_factory):
    ...
    for rule_class in rule_factory.rules.values():
        assert issubclass(rule_class, Rule), f"{rule_class.__name__} is not a Rule subclass"
...
def test_test_generation(rule_factory):
    ...
    for rule_name, rule_class in rule_factory.rules.items():
        ...
        assert len(test_cases) == rule_factory.num_tests, \
            f"Incorrect number of test cases generated for {rule_name}"
        
        # Verify test case structure
        for test_case in test_cases:
            # Check inputs
            assert set(test_case.inputs.keys()) == set(rule.input_vars), \
                f"Test case inputs don't match rule inputs for {rule_name}"
            
            # Check expected outputs
            assert set(test_case.expected_outputs.keys()) == set(rule.output_vars), \
...
def test_rule_execution(rule_factory):
    ...
    assert set(results.keys()) == set(rule_factory.rules.keys()), \
    ...
    for rule_name, rule_results in results.items():
        ...
        assert len(rule_results) == rule_factory.num_tests, \
            f"Incorrect number of results for {rule_name}"
        
        # Each result should be properly formed
        for result in rule_results:
            assert hasattr(result, 'passed'), f"Result missing 'passed' attribute for {rule_name}"
            assert hasattr(result, 'test_case'), f"Result missing 'test_case' attribute for {rule_name}"
            assert result.rule_name == rule_name, f"Result has incorrect rule name for {rule_name}"
...
def test_rule_failure_detection(rule_factory):
    ...
    for rule_name, rule_results in results.items():
        ...
        for result in failed_tests:
            ...
            assert result.test_case.actual_outputs != result.test_case.expected_outputs, \
