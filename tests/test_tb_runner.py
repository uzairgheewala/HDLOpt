...
class TestTestbenchRunner:
    ...
    def test_recursive_run(self, runner, temp_testbench_dir):
        ...
        # Run recursive tests
        results = runner.run_recursive("test_component", base_dir=str(temp_testbench_dir.parent))
        
        # Verify results
        assert len(results) >= 2  # At least one result each for main component and submodule
