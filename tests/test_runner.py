...
from ..runner import (
    HDLAnalysisRunner, RunnerConfig, AnalysisType,
    main as runner_main
)
...
class TestHDLAnalysisRunner:
    ...
    def test_basic_initialization(self, temp_workspace):
        ...
        config = RunnerConfig(output_dir=str(temp_workspace / "generated"), src_dir=str(temp_workspace / "src"))
    ...
    def test_directory_creation(self, temp_workspace, mock_simulators, mock_analysis):
        ...
        assert (temp_workspace / "generated" / "full_adder" / "full_adder_timing_report.pdf").exists()
        assert (temp_workspace / "generated" / "full_adder" / "full_adder_power_report.pdf").exists()
        assert (temp_workspace / "generated" / "full_adder" / "full_adder_waveform_report.pdf").exists()
        assert (temp_workspace / "generated" / "full_adder" / "full_adder_schematic.pdf").exists()
        assert (temp_workspace / "generated" / "full_adder" / "full_adder_analysis.pdf").exists()
    ...
    def test_testbench_generation(self, temp_workspace, mock_simulators, mock_analysis):
        ...
        assert (temp_workspace / "generated" / "half_adder" / "tb__half_adder.v").exists()
    ...
    %%EOF'''
        ...
        with patch("subprocess.run"), \
            patch("hdlopt.scripts.analysis.timing.TimingAnalyzer.analyze", mock_timing_analysis), \
            patch("hdlopt.scripts.analysis.power.PowerAnalyzer.analyze", mock_power_analysis):
            runner.run(["counter"])

        # Verify PDF generation
        timing_report = temp_workspace / "generated" / "counter" / "counter_timing_report.pdf"
        power_report = temp_workspace / "generated" / "counter" / "counter_power_report.pdf"
        combined_report = temp_workspace / "generated" / "counter" / "counter_analysis.pdf"
...
class TestCommandLine:
    ...
    def test_cli_all_modules(self, temp_workspace, mock_simulators):
        ...
        with patch("sys.argv", [
            "runner.py", "analyze",
            "-o", str(temp_workspace / "generated"),
            "-src", str(temp_workspace / "src"),
            "-v"
        ]):
            with patch.dict(os.environ, {"HDLOPT_OUTPUT_DIR": str(temp_workspace / "generated")}):
                runner_main()
    ...
    def test_cli_specific_analyses(self, temp_workspace, mock_simulators, mock_analysis):
        """Test running specific analyses from command line."""
        with patch("sys.argv", [
            "runner.py", "analyze", "counter",
            "-a", "timing", "power",
            "-o", str(temp_workspace / "generated"),
            "-src", str(temp_workspace / "src"),
            "-v"
        ]), patch("hdlopt.runner.HDLAnalysisRunner._run_analysis", side_effect=mock_analysis):
            runner_main()
    ...
    def test_cli_simulator_selection(self, temp_workspace, mock_simulators):
        ...
        iverilog_calls = [
            call_args for call_args, _ in mock_simulators.call_args_list
        ...
        assert any('vvp' in call_args[0] for call_args in iverilog_calls), "Missing vvp simulation command"
...
class TestExperimentTracking:
    ...
    def test_multiple_runs(self, temp_workspace, mock_simulators, mock_analysis):
        ...
        assert len(comparison["component_changes"]) > 0  # Should detect different modules
    ...
    def test_cli_experiment_options(self, temp_workspace, mock_simulators, mock_analysis):
        ...
        with patch("sys.argv", [
            ...
            "-n", "cli_test",
            "--version", "2.0",
            "-d", "CLI test experiment",
            "-t", "type=cli", "testcase=basic",
            "-o", str(temp_workspace / "generated"),
            "-src", str(temp_workspace / "src")
        ]), patch("hdlopt.runner.HDLAnalysisRunner._run_analysis", side_effect=mock_analysis):
            runner_main()
            ...
            try:
                with patch("sys.argv", ["runner.py"] + args):
                    with patch("argparse.ArgumentParser.parse_args", 
                        ...
                        runner.list_runs() if args[0] == 'list-runs' else \
                        runner.show_run_details(args[1]) if args[0] == 'show-run' else \
                        runner.show_component_history(args[1])
