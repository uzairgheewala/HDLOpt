...
class TestTestOptimizer:
    ...
    def test_calculate_module_complexity(self, temp_optimizer, sample_module_details):
        ...
        seq_complexity = temp_optimizer.calculate_module_complexity(sample_module_details)
    ...
    def test_identify_edge_cases(self, temp_optimizer, sample_input_ranges):
        ...
        edge_cases = temp_optimizer.identify_edge_cases(sample_input_ranges, special_signals)
        ...
        for case in edge_cases:
            ...
            for signal, value in case.items():
                ...
                is_edge_case = (
                    value in (min_val, max_val) or  # Min/Max
                    value == (min_val + max_val) // 2 or  # Mid
                    value in (min_val + 1, max_val - 1) or  # Near boundaries
                    (value & (value + 1)) == 0  # Power of 2 minus 1
                )
                assert is_edge_case
    ...
    def test_parallel_generate_testbenches(self, mock_executor, temp_optimizer, sample_module_details):
        ...
        with patch('hdlopt.scripts.testbench.core.TestbenchGenerator.generate') as mock_generate, \
         patch('pathlib.Path.stat') as mock_stat:
            ...
            os.makedirs(os.path.join(temp_optimizer.base_path, "test_adder"), exist_ok=True)

            temp_optimizer.parallel_generate_testbenches(
                test_batches,
                sample_module_details,
                Path(temp_optimizer.base_path)
            )
    ...
    def test_parallel_execute_testbenches(self, mock_executor, temp_optimizer):
        ...
        with patch('hdlopt.scripts.testbench.optimizer.Runner', return_value=mock_runner):
    ...
    def test_coverage_visualization(self, temp_optimizer, sample_input_ranges):
        ...
        test_cases = temp_optimizer.generate_test_distribution(
            sample_input_ranges,
            100
        )
    ...
    @pytest.mark.parametrize("num_cases,available_time,expected_success", [
        (1000, 60, True),    # Should fit in time
        (100000, 1, False), # Too many cases for time
        (500, None, True),   # No time constraint
    ])
    def test_time_constrained_optimization(self, temp_optimizer, sample_module_details,
                                         sample_input_ranges, num_cases, available_time,
                                         expected_success):
