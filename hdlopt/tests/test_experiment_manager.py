        ...
        for attempt in range(5):
            try:
                if manager.db_path.exists():
                    os.close(
                        os.open(str(manager.db_path), os.O_RDONLY)
                    )  # Force handle close
...
def test_start_run(experiment_manager, sample_verilog):
    ...
    run_id = experiment_manager.start_run(
        components=sample_verilog, config={"test_param": True}
    )
    ...
    with sqlite3.connect(experiment_manager.db_path) as conn:
        cursor = conn.execute(
            "SELECT component_name FROM component_history WHERE run_id = ?", (run_id,)
        )
...
def test_component_history(experiment_manager, temp_dir):
    ...
    run1_id = experiment_manager.start_run(components=[v1_file], config={})

    # Create modified version
    v1_file.write_text("module test; assign x = 2; endmodule")

    run2_id = experiment_manager.start_run(components=[v1_file], config={})
...
def test_compare_runs(experiment_manager, temp_dir):
    ...
    run1_id = experiment_manager.start_run(components=[module_a, module_b], config={})
    ...
    run2_id = experiment_manager.start_run(components=[module_a, module_b], config={})
...
def test_get_latest_version(experiment_manager, temp_dir):
    ...
    for i in range(3):
        ...
        run_id = experiment_manager.start_run(components=[module], config={})
...
def test_concurrent_runs(temp_dir):
    ...
    for i in range(3):
        ...
        config = ExperimentConfig(name=f"test_{i}", version="1.0", base_path=subdir)
...
def test_file_cleanup(temp_dir):
    ...
    config = ExperimentConfig(name="cleanup_test", version="1.0", base_path=temp_dir)
    ...
    run_id = manager.start_run(components=test_files, config={})
...
def test_large_experiment(experiment_manager, temp_dir):
    ...
    run_id = experiment_manager.start_run(components=test_files, config={})
...
def test_database_consistency(experiment_manager, sample_verilog):
    ...
    def count_records():
        with sqlite3.connect(experiment_manager.db_path) as conn:
            ...
            history = conn.execute("SELECT COUNT(*) FROM component_history").fetchone()[
                0
            ]
            return runs, history
