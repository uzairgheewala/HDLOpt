...
from ..scripts.analysis.schematic import (
    SchematicConfig, SchematicGenerator, SchematicFormat,
    SchematicTool, SchematicLevel, SchematicOrientation
)
from ..scripts.reporting.templates.schematic import (
    SchematicTemplate, SchematicMetrics, SchematicAnnotation
)
    ...
    with patch("subprocess.run") as mock_run:
        def side_effect(*args, **kwargs):
            ...
            if isinstance(args[0], (list, tuple)) and "vivado" in args[0][0].lower():
                ...
                match = re.search(r"write_schematic\s+-force\s+-format\s+(\S+)\s+(.+)", tcl_content)
                if match:
...
def test_yosys_generation(temp_component_dir, schematic_config):
    ...
    with patch("subprocess.run") as mock_run:
        ...
        gen = SchematicGenerator("full_adder", schematic_config, base_dir=str(temp_component_dir))
...
def test_error_handling(temp_component_dir, schematic_config):
    ...
    with pytest.raises(subprocess.CalledProcessError):
        gen = SchematicGenerator("nonexistent", schematic_config, base_dir=str(temp_component_dir))
        gen.generate()
...
class TestSchematicTemplate:
    ...
    def test_template_generation(
        self, pdf_report, temp_component_dir,
        sample_metrics, sample_annotations, sample_schematic
    ):
