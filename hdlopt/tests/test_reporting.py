...
class TestTestbenchSummaryTemplate:
    ...
    def test_all_tests_passed(self, pdf_generator):
        ...
        data = {
            "test_component": [
                {"expected": {"out": i}, "actual": {"out": i}} 
                for i in range(10)
            ]
        }
        
        elements = template.generate_page(data)
        table_data = [elem for elem in elements if isinstance(elem, Table)][0]._cellvalues
