...
def mock_pyverilog_ast():
    ...
    from pyverilog.vparser.ast import (
        Source, Description, ModuleDef, Node, 
        Paramlist, Portlist, Port, Width, 
        IntConst, Input, Output, Identifier
    )
    ...
    module = ModuleDef(
        ...
        portlist=Portlist([
            ...
            Port(name="a", width=Width(msb=IntConst("7"), lsb=IntConst("0")), type="wire", sign=False)
        ]),
        items=[]
    )
...
class TestPyVerilogParser:
    ...
    @pytest.mark.parametrize("signal_text,expected_error", [
