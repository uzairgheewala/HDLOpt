...
class ResourceAnalyzer:
    ...
    def _collect_source_files(self) -> List[str]:
        ...
        return [str(f) for f in src_dir.rglob("*.v") if not f.name.startswith("tb_")]
    ...
    def _analyze_netlist(self, netlist: Dict) -> Dict:
        ...
        for param_config in param_configs:
            ...
            analysis = self.netlist_analyzer.analyze(
                netlist, self.component_name, param_config, self.config
            )
            ...
            if details_file.exists():
                with open(details_file) as f:
                    ...
                    if "parameters" in details:
                        configs.append(
                            {p["name"]: int(p["value"]) for p in details["parameters"]}
                        )
    ...
    def _collect_raw_gates(
        self, module_info: Dict, netlist: Dict, raw_gates: Dict
    ) -> None:
        ...
        for cell in module_info["cells"].values():
            ...
            if cell_type in netlist["modules"]:
                ...
                self._collect_raw_gates(
                    netlist["modules"][cell_type], netlist, raw_gates
                )
            else:
