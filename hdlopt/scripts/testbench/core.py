...
class TestbenchGenerator:
    ...
    def _process_parameters(self, component_details: Dict) -> List[tuple]:
        ...
        for param in parameters:
            ...
            if value <= 0:
                raise ValueError(f"Parameter {name} value must be positive, got {value}")
    ...
    def _validate_inputs(self, input_ranges: Dict[str, List[int]], component_details: Dict) -> Dict[str, List[int]]:
        """Apply input constraints and validation"""
        if not self.constraints.input_constraints:
            return input_ranges
            
        component_constraints = self.constraints.input_constraints.get(self.component_name, {})
    ...
    def _generate_test_cases(self, input_ranges: Dict[str, List[int]], num_tests: int) -> List[Dict[str, int]]:
        ...
        while len(test_cases) < num_tests:
            ...
            for signal_name, signal_range in input_ranges.items():
                test_case[signal_name] = random.randint(signal_range[0], signal_range[1])
    ...
    def _assign_rules(self, test_cases: List[Dict[str, int]], component_details: Dict) -> List[Dict[str, int]]:
                ...
                if rule.matches(component_details['component_name']):
                    ...
                    try:
                        if isinstance(rule, MockRule):
                            ...
                            inputs = {}
                            non_special_inputs = [k for k in test_case.keys() 
                        ...
                        logger.error(f"Error generating expected value: {str(e)}", exc_info=True)
                        raise
    ...
    def generate(self, recursive: bool = False, test_cases: Optional[List[Dict[str, int]]] = None) -> None:
        ...
        param_names, param_combinations, _ = self._get_param_combinations(param_ranges, self.constraints.param_constraints)
    ...
    def _generate_single_testbench_with_cases(self, 
                                            param_comb: tuple,
        ...
        component_details = self._update_bit_widths(component_details, param_comb, param_names)
    ...
    def _generate_single_testbench(self, param_comb: tuple, param_names: List[str], 
        ...
        component_details = self._setup_special_signals(component_details)
        logger.debug(f"Special signals setup: start={component_details.get('start_present')}, valid={component_details.get('valid_present')}")
    ...
    def _save_testbench(self, tb_code: str, param_comb_str: str) -> None:
        ...
        component_dir = self._find_component_dir()
        tb_filename = os.path.join(component_dir, 
                                 f"tb_{param_comb_str}_{self.component_name}.v")
    ...
    def _update_component_details(self, component_details: Dict, param_comb_str: str, 
                            test_cases: List[Dict], expected_values: List[Dict]) -> None:
        ...
        for test_case in test_cases:
            ...
            for input, value in test_case.items():
                if input not in invalid_inputs:
                    value_str = self._get_signal_value_str(input, value, component_details)
