...
class IntegratedTestManager:
    ...
    def plan_tests(self, 
                  module_details: Dict,
                  desired_cases: int,
                  available_time: Optional[float] = None) -> TestExecutionPlan:
        ...
        if available_time:
            ...
            if est_time > available_time:
                ...
                logger.debug(f"Reduced to {len(regular_cases)} cases to meet time constraint")
                est_time = available_time
        else:
            est_time = self.optimizer.estimate_execution_time(complexity, len(edge_cases) + len(regular_cases))
    ...
    def execute_test_plan(self,
                         ...
                         module_details: Dict,
                         recursive: bool = False) -> List[TestbenchResult]:
    ...
    def _generate_coverage_artifacts(self,
                                   plan: TestExecutionPlan,
