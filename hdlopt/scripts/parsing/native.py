...
class NativeVerilogParser(VerilogParserBase):
            ...
            if os.path.exists(file_or_content):
                try:
                    ...
                    raise FileProcessingError(
                        f"Failed to read file {file_or_content}: {str(e)}"
                    )
            else:
    ...
    def _parse_signal(
        ...
        if len(directions) > 1:
            raise VerilogParsingError(
                f"Contradictory signal directions in: {declaration}"
            )
        ...
        if len(signal_types) > 1:
            raise VerilogParsingError(
                f"Multiple signal types specified in: {declaration}"
            )
        ...
        if default_value is None:
            default_value = self._generate_default_value(
                sig_type, bit_width, parameters or [], is_input
            )
    ...
    def _parse_module_text(self, module_text: str) -> Optional[VerilogModule]:
            ...
            for i, line in enumerate(lines, 1):
                ...
                while pos < len(line):
                    ...
                    # Handle end blocks (but not endmodule)
                        ...
                        pos + 2 < len(line)
                        and line[pos : pos + 3] == "end"
                        and (pos + 8 >= len(line) or line[pos : pos + 9] != "endmodule")
                    ):
                        if not stack:
        ...
        def validate_port_list(text: str) -> None:
            ...
            if param_start != -1:
                ...
                logger.debug(
                    f"No parameter list, found port list start at: {port_list_start}"
                )
            ...
            while paren_count > 0 and port_end_pos < endmodule_pos:
                ...
                if text[port_end_pos] == "(":
                    ...
                    paren_count -= 1
            logger.debug(
                f"Found port list closing parenthesis at position: {port_end_pos}"
            )
            ...
            if next_pos is None:
                logger.debug(
                    "No significant characters found after port list closing parenthesis"
                )
                raise ModuleDefinitionError("Port list must be followed by ';'")

            logger.debug(
                f"Found next significant character '{next_char}' at position {next_pos}"
            )
                            ...
                            if sig_type == "reg":
                                ...
                                if ":" in width:
                                    ...
                                    try:
                                        ...
                                        default_value = (
                                            "'b0"  # Fallback for parametric widths
                                        )
                                else:
                                    default_value = "'b0"
                            else:
                                ...
                                if ":" in width:
                                    ...
                                    try:
                                        ...
                                        default_value = (
                                            "'bz"  # Fallback for parametric widths
                                        )
                                else:
                                    default_value = "'bz"
    ...
    def _parse_parameters(self, param_text: str) -> List[Dict[str, str]]:
        ...
        for param_def in param_defs:
            ...
            if len(parts) == 2:
                ...
                raise ModuleDefinitionError(
                    f"Parameter must have a default value: {param_def}"
                )
    ...
    def _finalize_block(
        self, block_lines: List[str], block_type: str, module: VerilogModule
    ) -> None:
        ...
        try:
            ...
            if "][" in bit_width:  # Multi-dimensional
                ...
                if bit_width.isdigit():
                    ...
                    param = next(
                        (p for p in parameters if p["name"] in bit_width), None
                    )
                    if param:
