...
class PowerAnalyzer:
    ...
    """
        ...
        try:
            result = subprocess.run(
                ["vivado", "-mode", "batch", "-nolog", "-nojournal", "-source", str(script_path)],
                check=False,
                capture_output=True,
                text=True
            )
    ...
    def _generate_tcl_script(self) -> str:
        ...
        source_files = [str(f) for f in self._collect_source_files()]  # Convert paths to strings
        script = [
            ...
            if '1.1 On-Chip Components' in line:
                ...
                if len(parts) == 5:
                    try:
                        ...
                        components.append(ComponentPower(
                            name=parts[0],
                            power=power,
                            used=used,
                            available=available,
