...
class WaveformTemplate(PageTemplate):
    ...
    def generate_page(self, component_name: str, analysis_results: dict, plots: dict) -> None:
    ...
    def _add_metrics_summary(self, metrics):
        ...
        self.add_element(table)
        self.add_element(Paragraph("Signal activity analysis", self.styles['WaveformCaption']))
        ...
        data.extend([
            ["Glitches", len(glitches), f"Min Width: {min([g['width'] for g in glitches] or [0]):.2f} ns" if glitches else "No glitches"],
            ["Timing Violations", len(violations), self._format_violation_details(violations)]
        ])
        ...
        table.setStyle(style)
        self.add_element(table)
        self.add_element(Paragraph("Timing event summary", self.styles['WaveformCaption']))
        self.add_element(Spacer(1, 0.3*inch))
    ...
    def _add_waveform_plots(self, plots):
        ...
        if not plots:
            self.add_element(Paragraph("No waveform plots available.", self.styles['BodyText']))
            return
        ...
        if "singles" in plots:
            ...
            self.add_element(Spacer(1, 0.2*inch))
            self.add_element(Paragraph("Digital signal waveforms over time", self.styles['WaveformCaption']))
            self.add_element(Spacer(1, 0.3*inch))

        # Bus signals
        bus_plots = {k:v for k,v in plots.items() 
                    if k not in ["singles", "events"]}
        if bus_plots:
            ...
            for bus_name, plot_data in bus_plots.items():
                ...
                self.add_element(img)
                self.add_element(Spacer(1, 0.2*inch))
                self.add_element(Paragraph(f"Bus: {bus_name}", self.styles['WaveformCaption']))
                self.add_element(Spacer(1, 0.3*inch))
        ...
        if "events" in plots:
            ...
            self.add_element(img)
            self.add_element(Spacer(1, 0.2*inch))
            self.add_element(Paragraph("Timeline of timing events", self.styles['WaveformCaption']))
            self.add_element(Spacer(1, 0.3*inch))
